...
extern "C"
{
  ...
  /**
   * @brief    Creates a Provisioning Client for communications with the Device Provisioning Client
   * Service
   *
   ...
   *
   * @param    handle              The handle created by a call to the create function.
   * @param    register_callback   The callback that gets called on registration or if an error is
   * encountered
   * @param    user_context        User specified context that will be provided to the callback
   * @param    reg_status_cb       An optional registration status callback used to inform the
   * caller of registration status
   * @param    status_user_ctext   User specified context that will be provided to the registration
   * status callback
   *
  ...
  /**
   * @brief    API sets a runtime option identified by parameter optionName to a value pointed to by
   * value
   *
   * @param    handle          The handle created by a call to the create function.
   ...
   *
   * @param    handle          The handle created by a call to the create function.
   * @param    json            The data field that is sent to the service.  Setting json to NULL
   * will unset the value previously set
   *
   * @return PROV_DEVICE_RESULT_OK upon success or an error code upon failure
   */
