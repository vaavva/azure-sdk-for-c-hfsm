...
extern "C"
{
   ...
   *   @deprecated      This callback type is deprecated.  Use the callback
   * IOTHUB_CLIENT_FILE_UPLOAD_GET_DATA_CALLBACK_EX() instead.
   */
  ...
  /** @brief   Signature of the callback that the application implements to receive notifications of
   * module initiated device method calls.
   *
   *   @param   result              Result of the operation
   *   @param   responseStatus      HTTP status code returned from the module or device.  This is
   * only valid if @c result is @c IOTHUB_CLIENT_OK.
   *   @param   responsePayload     HTTP response payload returned from the module or device.  This
   * is only valid if @c result is @c IOTHUB_CLIENT_OK.
   *   @param   responsePayloadSize Number of bytes in @c responsePayload.  This is only valid if @c
   * result is @c IOTHUB_CLIENT_OK.
   *   @param   context             Context value passed is initial call to
   * IoTHubModuleClient_ModuleMethodInvokeAsync() (e.g.).
   *
   *   @remarks Module clients when hosted in IoT Edge may themselves invoke methods on either
   * modules on the same IoT Edge device or on downstream devices using APIs such as
   * IoTHubModuleClient_LL_DeviceMethodInvoke() or IoTHubModuleClient_LL_ModuleMethodInvoke(). These
   * APIs operate asynchronously.  When the invoked module or device returns (or times out), the IoT
   * Hub SDK will invoke the application's IOTHUB_METHOD_INVOKE_CALLBACK() callback.
   *
   *   @warning This API is only applicable to applications running inside a module container hosted
   * by IoT Edge.  Calling outside a IoT Edge hosted edge container will result in undefined
   * results.
   */
  ...
  /** @brief Enumeration returned by the GetSendStatus family of APIs (e.g.
   * IoTHubDeviceClient_LL_GetSendStatus()) to indicate the current sending status of the IoT Hub
   * client.
   */
    typedef enum
    {
    ...
    */
    typedef enum IOTHUB_CLIENT_CONNECTION_STATUS_REASON
    {
  ...
  /**
   *   @brief Signature of the callback that the application implements to process connection status
   * changes between device and IoT Hub.
   *
   *   @param result                  Whether device is successfully connected (@c
   * IOTHUB_CLIENT_CONNECTION_AUTHENTICATED) or not (@c IOTHUB_CLIENT_CONNECTION_UNAUTHENTICATED).
   *   @param reason                  More information about @c result, especially if connection was
   * unsuccessful.
   *   @param userContextCallback     Context that application specified during initial API call to
  ...
  /**
   *   @brief Signature of the callback that the application implements to process incoming
   * cloud-to-device messages.
   *
   *   @param message                 The incoming message received from IoT Hub.
   *   @param userContextCallback     Context that application specified during initial API call to
   * receive incoming cloud-to-device messages.
   *
   *   @return #IOTHUBMESSAGE_DISPOSITION_RESULT indicating how client has acknowledged the incoming
  ...
  /**
   *   @brief   Signature of the callback that the application implements to process data received
   * from an IoT Hub device or module twin.
   *
   *   @param   update_state          Whether application has received a full twin or just a patch
   * update.
  ...
  /**
   *   @brief Signature of the callback that the application implements to receive notifications of
   * device or module initiated twin updates.
   *
   *   @param status_code             HTTP style status code indicating success or failure of
   * operation.
   *   @param userContextCallback     Context that application specified during initial API call to
   * update twin data.
   */
  ...
  /**
   *   @brief   Signature of the callback that the application implements to receive incoming device
   * or module method invocations from IoT Hub
   *
   *   @param   method_name           Name of method being invoked.
   *   @param   payload               Request payload received from IoT Hub.
   *   @param   size                  Number of bytes in @c payload.
   *   @param   response              Response of the request, as specified by the application. This
   * should NOT include the null-terminator.
   *   @param   response_size         Number of bytes application specifies in @c response.
   *   @param   userContextCallback   Context that application specified during initial API call to
   * receive device or module method calls.
   *
   *   @remarks The application should allocate @c response with @c malloc.  The IoT Hub client SDK
   * will @c free the data automatically.
   *
   *   @warning The data in @c payload is not guaranteed to be a null-terminated string.
   *
  ...
  /** @brief           Return value applications use in their implementation of
   * IOTHUB_CLIENT_FILE_UPLOAD_GET_DATA_CALLBACK_EX() to indicate status.
   */
  typedef enum
  {
  ...
  /**
   *  @brief           Signature of the callback that the application implements to process IoT Hub
   * client SDK requesting additional chunks to upload to blob.
   *
   *  @param result    The result of the upload of the previous block of data provided by the user.
   *  @param data      Next block of data to be uploaded, to be provided by the user when this
   * callback is invoked.
   *  @param size      Size of the data parameter.
   *  @param context   User context provided on the call to
   * IoTHubClient_UploadMultipleBlocksToBlobAsync.
   *
   *  @return          #IOTHUB_CLIENT_FILE_UPLOAD_GET_DATA_RESULT indicating whether the application
   * is returning data to be sent or not.
   *
   *  @remarks         If the user wants to abort the upload, the callback should return
   * IOTHUB_CLIENT_FILE_UPLOAD_GET_DATA_ABORT It should return IOTHUB_CLIENT_FILE_UPLOAD_GET_DATA_OK
   * otherwise. If a NULL is provided for parameter "data" and/or zero is provided for "size", the
   * user indicates to the client that the complete file has been uploaded. In such case this
   * callback will be invoked only once more to indicate the status of the final block upload. If
   * result is not FILE_UPLOAD_OK, the upload is cancelled and this callback stops being invoked.
   *                   When this callback is called for the last time, no data or size is expected,
   * so data and size are NULL
   */
  typedef IOTHUB_CLIENT_FILE_UPLOAD_GET_DATA_RESULT (
  ...
  {
    /** @brief A function pointer that is passed into IoTHubDeviceClient_CreateWithTransport() or
     * IoTHubDeviceClient_LL_Create(). A function definition for MQTT is defined in the include @c
    ...
    /** @brief    Optional gateway host to connect to (instead of directly to IoT Hub).  Can be
     * NULL. */
