 ...
 *              https://github.com/Azure/azure-iot-sdk-c/blob/main/doc/threading_notes.md for more
 * details.
 *
 */
...
extern "C"
{
   ...
   *
   *  @warning The API functions that use this handle are not thread safe.  See
   ...
   *                <blockquote>
   *                    <pre>HostName=[IoT Hub name goes here].[IoT Hub suffix goes here, e.g.,
   * private.azure-devices-int.net];DeviceId=[Device ID goes here];SharedAccessKey=[Device key goes
   * here];</pre>
   *                </blockquote>
   *
  ...
  IOTHUB_DEVICE_CLIENT_LL_HANDLE IoTHubDeviceClient_LL_CreateWithTransport(
      const IOTHUB_CLIENT_DEVICE_CONFIG* config);

  /**
   * @brief    Creates a IoT Hub client for communication with an existing IoT
   *           Hub using the device auth.
   *
   * @param    iothub_uri             Pointer to an IoT Hub hostname received in the registration
   * process
   ...
   *           so this argument can be destroyed by the calling application right after
   * IoTHubDeviceClient_LL_SendEventAsync returns. The copy of @c eventMessageHandle is later
   * destroyed by the iothub client when the message is effectively sent, if a failure sending it
   * occurs, or if the client is destroyed.
   ...
   *                                     if there are.
   *
   * @remark    Does not return information related to uploads initiated by
   ...
   *                                           updates about the status of the connection to IoT
   * Hub.
   * @param    userContextCallback             User specified context that will be provided to the
   *                                           callback. This can be @c NULL.
   *
   ...
   *           the IoTHubDeviceClient_LL_Destroy() function from within any callback.
   *
   * @remark   Callback specified will not receive connection status change notifications for upload
   * connections created with IoTHubDeviceClient_LL_UploadToBlob() or
   ...
   *                                           connection drops.
   * @param    retryTimeoutLimitInSeconds      Maximum amount of time(seconds) to attempt
   * reconnection when a connection drops to IOT Hub.
   *
   *           @b NOTE: The application behavior is undefined if the user calls
   *           the IoTHubDeviceClient_LL_Destroy() function from within any callback.
   *
   * @remark   Uploads initiated by IoTHubDeviceClient_LL_UploadToBlob() or
   * IoTHubDeviceClient_LL_UploadMultipleBlocksToBlob() do not have automatic retries and do not
   * honor the retryPolicy settings.
   *
   * @return   IOTHUB_CLIENT_OK upon success or an error code upon failure.
   */
  ...
  * the connection to IOT Hub. This is a blocking call.
  *
  * @param    iotHubClientHandle                  The handle created by a call to the create
  function.
  * @param    retryPolicy                         Out parameter containing the policy to use to
  reconnect to IoT Hub.
  * @param    retryTimeoutLimitInSeconds          Out parameter containing maximum amount of time in
  seconds to attempt reconnection to IOT Hub.
  *
  *           @b NOTE: The application behavior is undefined if the user calls
  *           the IoTHubDeviceClient_LL_Destroy() function from within any callback.
  *
  ...
  */
  ...
  /**
   * @brief    This function MUST be called by the user so work (sending/receiving data on the
   * network, computing and enforcing timeout controls, managing the connection to the IoT Hub) can
   ...
   *
   * @remarks  Documentation for configuration options is available at
   * https://github.com/Azure/azure-iot-sdk-c/blob/main/doc/Iothub_sdk_options.md.
   *
   * @return   IOTHUB_CLIENT_OK upon success or an error code upon failure.
   */
  ...
  /**
   * @brief   This API specifies a callback to be used when the device receives a desired state
   * update.
   *
   * @param   iotHubClientHandle        The handle created by a call to the create function.
   * @param   deviceTwinCallback        The callback specified by the device client to be used for
   * updating the desired state. The callback will be called in response to patch request send by
   * the IoT Hub services. The payload will be passed to the callback, along with two version
   * numbers:
   *                                        - Desired:
   *                                        - LastSeenReported:
   * @param   userContextCallback       User specified context that will be provided to the
   ...
   *
   * @param    iotHubClientHandle        The handle created by a call to the create function.
   * @param    reportedState             The current device property values to be 'reported' to the
   * IoT Hub.
   * @param    size                      Number of bytes in @c reportedState.
   * @param    reportedStateCallback     The callback specified by the device client to be called
  ...
  /**
   * @brief	This API enabled the device to request the full device twin (with all the desired and
   *reported properties) on demand.
   *
   * @param	iotHubClientHandle		The handle created by a call to the create function.
   * @param	deviceTwinCallback	    The callback specified by the device client to receive the
   *Twin document.
   * @param	userContextCallback		User specified context that will be provided to the
   * 									callback. This can be @c
   *NULL.
   *
   *			@b NOTE: The application behavior is undefined if the user calls
   ...
   * @param    userContextCallback                User specified context that will be provided to
   * the callback. This can be @c NULL.
   *
   * @return   IOTHUB_CLIENT_OK upon success or an error code upon failure.
   */
  ...
  /**
   * @brief    This API uploads to Azure Storage the content pointed to by @p source having the size
   * @p size under the blob name @p devicename/destinationFileName
   *
   * @param    iotHubClientHandle      The handle created by a call to the create function.
   * @param    destinationFileName     name of the file.
   * @param    source                  pointer to the source for file content (can be NULL)
   * @param    size                    the size of the source in memory (if @p source is NULL then
   * size needs to be 0).
   *
   * @warning  Other _LL_ functions such as IoTHubDeviceClient_LL_SendEventAsync() queue work to be
   * performed later and do not block.  IoTHubDeviceClient_LL_UploadToBlob will block however until
   * the upload is completed or fails, which may take a while.
   *
   * @return   IOTHUB_CLIENT_OK upon success or an error code upon failure.
   */
  ...
  /**
   * @brief    This API uploads to Azure Storage the content provided block by block by @p
   * getDataCallback under the blob name @p devicename/destinationFileName
   *
   * @param    iotHubClientHandle      The handle created by a call to the create function.
   * @param    destinationFileName     name of the file.
   * @param    getDataCallbackEx       A callback to be invoked to acquire the file chunks to be
   * uploaded, as well as to indicate the status of the upload of the previous block.
   * @param    context                 Any data provided by the user to serve as context on
   * getDataCallback.
   *
   * @warning  Other _LL_ functions such as IoTHubDeviceClient_LL_SendEventAsync() queue work to be
   * performed later and do not block.  IoTHubDeviceClient_LL_UploadMultipleBlocksToBlob will block
   * however until the upload is completed or fails, which may take a while.
   *
   * @return   IOTHUB_CLIENT_OK upon success or an error code upon failure.
   */
  ...
  /**
   * @brief    This API sends an acknowledgement to Azure IoT Hub that a cloud-to-device message has
   * been received and frees resources associated with the message.
   *
   * @param    device_ll_handle                The handle created by a call to a create function.
   * @param    message                         The cloud-to-device message received through the
   * callback provided to IoTHubDeviceClient_LL_SetMessageCallback.
   * @param    disposition                     Acknowledgement option for the message.
   *
   * @warning  This function is to be used only when IOTHUBMESSAGE_ASYNC_ACK is used in the callback
   * for incoming cloud-to-device messages.
   * @remarks
   *           If your cloud-to-device message callback returned IOTHUBMESSAGE_ASYNC_ACK, it MUST
   * call this API eventually. Beyond sending acknowledgment to the service, this method also
   * handles freeing message's memory. Not calling this function will result in memory leaks.
   *           Depending on the protocol used, this API will acknowledge cloud-to-device messages
   * differently: AMQP: A MESSAGE DISPOSITION is sent using the `disposition` option provided. MQTT:
   * A PUBACK is sent if `disposition` is `IOTHUBMESSAGE_ACCEPTED`. Passing any other option results
   * in no PUBACK sent for the message. HTTP: A HTTP request is sent using the `disposition` option
   * provided.
   * @return   IOTHUB_CLIENT_OK upon success, or an error code upon failure.
   */
