...
az_result az_iot_hub_twin_response_subscribe_topic_get(az_span mqtt_topic_filter, az_span* out_mqtt_topic_filter);
...
// az_result encapsulates the status.
az_result az_iot_twin_handle(az_span received_topic, az_iot_hub_twin_response* out_twin_response);

// Requires an empty MQTT payload.
// TODO: better name?
az_result az_iot_twin_get_publish_topic_get(az_span request_id, az_span mqtt_topic, az_span *out_mqtt_topic);

// MQTT payload contains a JSON document formatted according to the Twin specification.
az_result az_iot_twin_patch_publish_topic_get(az_span request_id, az_span mqtt_topic, az_span *out_mqtt_topic);
