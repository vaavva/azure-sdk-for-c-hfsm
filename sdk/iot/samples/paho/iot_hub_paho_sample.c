...
#define TIMEOUT 4 * 60 * 1000L // Recommended 4 minute timeouts for TCP operations
...
int msgarrvd(void * context, char * topicName, int topicLen, MQTTClient_message * message) {
  ...
  if (az_succeeded(ret = az_iot_c2d_handle(pub_received, &c2d_request))) {
    ...
    printf(
        "Method called: %s with payload_size: %d\n",
...
int main(int argc, char * argv[]) {
  ...
  if (!az_succeeded(az_iot_hub_user_name_get(
          AZ_SPAN_FROM_STR("myhub.azuredevices.com"), &identity, az_span_null(),
    ...
    MQTTClient_create(
        &client, AZ_IOT_HUB_HOST_NAME, AZ_DEVICE_ID, MQTTCLIENT_PERSISTENCE_NONE, NULL);
  ...
  // Create client
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if (!az_succeeded(
              rc = az_iot_sendtelemetry(
                  iot_client, payload, properties, ARRAY_SIZE(properties), &mqtt_pub))) {
