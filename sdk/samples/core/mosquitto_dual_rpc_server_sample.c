...
/**
 * @file
 * @brief Mosquitto dual rpc server sample
 *
 */
...
// pending command
static char unlock_correlation_id_buffer[AZ_MQTT5_RPC_CORRELATION_ID_LENGTH];
...
// pending command
static char getstats_correlation_id_buffer[AZ_MQTT5_RPC_CORRELATION_ID_LENGTH];
...
int main(int argc, char* argv[])
{
  ...
  // az_mqtt5_rpc_server_memory unlock_rpc_server_memory
  //     = (az_mqtt5_rpc_server_memory){ .property_bag = unlock_property_bag,
  //                                     .sub_topic = AZ_SPAN_FROM_BUFFER(unlock_sub_topic_buffer) };

  LOG_AND_EXIT_IF_FAILED(az_rpc_server_init(&unlock_rpc_server, &mqtt_connection, unlock_property_bag, AZ_SPAN_FROM_BUFFER(unlock_sub_topic_buffer), model_id, client_id, unlock_command_name, NULL));
  ...
  // az_mqtt5_rpc_server_memory getstats_rpc_server_memory
  //   = (az_mqtt5_rpc_server_memory){ .property_bag = getstats_property_bag,
  //                                   .sub_topic = AZ_SPAN_FROM_BUFFER(getstats_sub_topic_buffer) };

  LOG_AND_EXIT_IF_FAILED(az_rpc_server_init(&getstats_rpc_server, &mqtt_connection, getstats_property_bag, AZ_SPAN_FROM_BUFFER(getstats_sub_topic_buffer), model_id, client_id, getstats_command_name, NULL));
