...
AZ_NODISCARD az_result _az_event_pipeline_post_outbound_event(
    _az_event_pipeline* pipeline,
...
AZ_NODISCARD az_result _az_event_pipeline_post_inbound_event(
    _az_event_pipeline* pipeline,
    az_event const event)
{
  az_result ret;
  ...
  if (az_result_failed(ret))
  {
    ret = _az_event_pipeline_post_inbound_event(
        ...
        (az_event){ .type = AZ_HFSM_EVENT_ERROR,
                    .data = &(_az_hfsm_event_data_error){
                        .error_type = ret,
...
AZ_NODISCARD az_result _az_event_pipeline_timer_create(
    _az_event_pipeline* pipeline,
    _az_event_pipeline_timer* out_timer)
{
